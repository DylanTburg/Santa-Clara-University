...
//swaps x and y (i and j). O(1). Also my indentation is different to the rest of the file, but that's the only way it will let me indent.
...
//partitiom for quicksort. O(n)
int partition(void ** elts, int (*compare)(), int low, int high) 
{
	...
	swap(elts[i + 1], elts[high]);
	return i + 1;
}
//quicksort function recursively. O(n^2) worst case. Average case is O(nlogn)
void quicksort_recurse(void ** elts, int (*compare)(), int low, int high) 
{
	if (low < high) 
	{
        	int x = partition(elts, compare, low, high);
        	quicksort_recurse(elts, compare, low, x - 1);
        	quicksort_recurse(elts, compare, x + 1, high);
	}
}
//sets up quicksort_recurse. O(1)
void quicksort(void ** elts,SET *sp, int (*compare)())
{
        quicksort_recurse(elts, compare, 0, sp->count - 1);
}
 ...
 * to by SP.
 * I've added this part. In theory the complexity should also be O(n^3)
 */
