...
//
//   Read ports of IMEM/DMEM now take the address-phase address (mem_raddr_ap) so
//   ahb_mem registers its read output and infers block RAM. Latency is unchanged.
//////////////////////////////////////////////////////////////////////////////////
...
module ahb_top (
    ...
    wire [31:0] mem0_raddr_ap;   // address-phase read address (registered inside ahb_mem -> BRAM)
    ...
    wire [31:0] mem1_raddr_ap;   // address-phase read address (registered inside ahb_mem -> BRAM)
    ...
    ahb_mem#(.INIT_FILE("insn_coverage.hex"), .WORDS(16384)) IMEM (  // 64KB Flash (STM32F051R8T6)
        .HCLK(HCLK),
        .mem_write(mem0_write),
        .mem_raddr_ap(mem0_raddr_ap),
        .mem_waddr(mem0_waddr),
    ...
    ahb_mem#(.INIT_FILE("dmem.hex"), .WORDS(2048)) DMEM (          // 8KB SRAM (STM32F051R8T6)
        .HCLK(HCLK),
        .mem_write(mem1_write),
        .mem_raddr_ap(mem1_raddr_ap),
        .mem_waddr(mem1_waddr),
    ...
    // Slaves 2 and 3 are unused: the memory map has a single DMEM (SRAM) on
    // slave 1. The mux's slave-2/3 inputs are tied off in its instantiation.
